...
business-candidate-analyst/tests/test_safety.py and its own precedents
(observation-agent, reality-sensor, headquarters). This package's whole
job is to hold hand-researched evidence without ever touching the
network, a model, or any file outside its own data/ and reports/ - this
...
# Intake is JSON-only (stdlib json), deliberately avoiding a YAML parser
# dependency - see README. Not a network concern, checked separately so
...
_CA_AGENTS_IMPORT = re.compile(r"^\s*(import\s+(ca_agents|business_candidate_analyst)\b|"
                                r"from\s+(ca_agents|business_candidate_analyst)\b)", re.MULTILINE)
...
class SafetyTests(unittest.TestCase):
        ...
        for path in _all_source_files():
            ...
            for needle in _FORBIDDEN_NETWORK:
                self.assertNotIn(needle, text,
                                  ...
                                  "never call a model or the network, and intake is JSON-only (no YAML parser "
                                  "dependency).")
    ...
    def test_at_least_the_expected_write_calls_exist(self):
        ledger_text = (_SRC_ROOT / "constraint_change_observatory" / "ledger.py").read_text(encoding="utf-8")
        report_text = (_SRC_ROOT / "constraint_change_observatory" / "report.py").read_text(encoding="utf-8")
        ...
        for path in _all_source_files():
            ...
            for needle in _FORBIDDEN_THIRD_PARTY:
                self.assertNotIn(needle, text,
                                  ...
                                  "no third-party dependency (repo convention, see CA README).")

    def test_no_module_imports_other_analyst_packages(self):
        """Independence boundary: this package must not import
        ca_agents (Constraint Archaeology) or business_candidate_analyst
        (including its Mode B) - it is a wholly separate evidence
        stream, per CONTRACT.md."""
        for path in _all_source_files():
            text = path.read_text(encoding="utf-8")
            self.assertIsNone(_CA_AGENTS_IMPORT.search(text),
    ...
    def test_ca_agents_import_pattern_actually_detects_a_violation(self):
        self.assertIsNotNone(_CA_AGENTS_IMPORT.search("from ca_agents.memory import classify_function"))
        self.assertIsNotNone(_CA_AGENTS_IMPORT.search("import business_candidate_analyst.analyst"))
