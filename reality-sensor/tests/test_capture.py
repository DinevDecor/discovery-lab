...
from reality_sensor.capture import load_raw_captures
...
class TestMalformedFeeds(unittest.TestCase):
    ...
    def test_missing_required_field_is_skipped_not_fatal(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(
                json.dumps([{"source_name": "X"}]),  # missing everything else
                encoding="utf-8",
            )
            captures, reasons = load_raw_captures(path)
            self.assertEqual(captures, [])
    ...
    def test_one_good_one_bad_entry_keeps_the_good_one(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            good = {
                ...
                "category": "FOUNDATION_MODEL_RELEASES",
                "captured_at": "2026-07-20T00:00:00Z",
                "title": "New model",
                "raw_text": "We released a new model.",
                "affected_capability": "new model",
            }
